...
if(isset($_POST['simpan'])){
    ...
    //cek dibagian db, apakah ada atau belum
    if($email != ''){
        $sql1 = "select email from members where email = '$email'";
        $q1   = mysqli_query($koneksi,$sql1);
        $n1   = mysqli_num_rows($q1);
        if($n1 > 0){
            $err .="<li>NPM yang kamu masukkan sudah terdaftar.</li>";
        }

        //validasi email
        if(!filter_var($email,FILTER_VALIDATE_EMAIL)){
            $err .= "<li>NPM yang kamu masukkan tidak valid.</li>";
        }
    }

    //cek kesesuaiaan password & konfirmasi password
    if($password != $konfirmasi_password){
        $err .= "<li>Pasword dan Konfirmasi Pasword tidak sesuai.</li>";
    }
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                ...
                <br/>
                Sudah punya akun? Silakan login <a href='<?php echo url_dasar()?>/login.php'>login</a>
            </td>
        </tr>
        <tr>
            <td></td>
